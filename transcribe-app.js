    ...
    pool.query(get_audio_url, (err, result) => {
      ...
      if (result && result.length > 0 && result[0]["audio_order"] != null) {
        ...
        pool.query(get_user_name, (err, userName_result) => {
          ...
          user_name = userName_result[0]
            ? userName_result[0]["name"]
            : "Not Found";
    ...
    await pool.query(get_audio_url, (err, result) => {
      ...
      if (result && result.length > 0) {
        ...
        pool.query(get_user_name, async (err, userName_result) => {
          ...
          if (
            userName_result &&
            userName_result.length > 0 &&
            typeof userName_result[0]["name"] != "undefined"
          ) {
...
app.post("/database", (req, res) => {
  let sql = `
            INSERT INTO posts(div_className, div_title, segment_start, segment_end, annotation_text, user_id, audio_id) VALUES('${
              req.body.speakerName
            }', '${req.body.annotationType}', '${req.body.segmentStart}', '${
    req.body.segmentEnd
...
app.post("/insert-into-actual-data", (req, res) => {
  let sql = `
            INSERT INTO actual(div_className, div_title, segment_start, segment_end, annotation_text, audio_id) VALUES('${
              req.body.speakerName
            }', '${req.body.annotationType}', '${req.body.segmentStart}', '${
    req.body.segmentEnd
  }',
...
app.post("/insert-into-transcription-tasks-segments", (req, res) => {
  ...
  pool.query(sql, (err, result) => {
    if (err) {
      ...
      res
        .status(400)
        .send("error in get /insert-into- transcription_task_segments query.");
    }
    // //console.log("Data Inserted Into Actual Table");
    res.send(result);
  });
...
app.post("/insert-into-transcription-table", (req, res) => {
  let sql = `
            INSERT INTO transcription(div_className, div_title, segment_start, segment_end, annotation_text, user_id, audio_id) VALUES('${
              req.body.speakerName
            }', '${req.body.annotationType}', '${req.body.segmentStart}', '${
    req.body.segmentEnd
  }', '${req.body.annotationText.replace(/'/g, "\\'")}', '${
...
app.post("/update-transcription-task-segment-table", (req, res) => {
  ...
  pool.query(sql, (err, result) => {
    if (err) {
      ...
      res
        .status(400)
        .send("error in get /update-transcription-task-segment-table query.");
    }
    //  //console.log("Updated");
  });
});
...
app.post("/update-transcription-table", (req, res) => {
  let sql = `
            ...
            SET  annotation_text = '${req.body.annotationText.replace(
              /'/g,
              "\\'"
            )}',div_className = '${req.body.speakerName}',div_title = '${
    req.body.annotationType
  }'
            WHERE segment_id = '${req.body.segmentId}'
            AND user_id = '${req.body.user_id}'
  ...
  pool.query(sql, (err, result) => {
    if (err) {
      ...
      res
        .status(400)
        .send(
          "error in get /update-transcription-task-segments-on-split query."
        );
    }
    ////console.log("Updated");
  });
  ...
  pool.query(reset_sql, (err, result) => {
    ...
    pool.query(setStatusNull, (resetErr, resetResult) => {
      if (resetErr) {
        ...
        res
          .status(400)
          .send("error in get /reset-transcription-data-for-retry.");
      }
...
app.post("/insert-feedback-lt", (req, res) => {
  let sql = `INSERT INTO feedbackLT (feedback,audio_id,feedbackCreatedAt,user_id) VALUES ("${req.body.feedback.replace(
    /'/g,
    "\\'"
  )}","${req.body.audio_id}",Now(),"${req.body.user_id}")`;
  pool.query(sql, (err, result) => {
    if (err) {
      console.error(err);
