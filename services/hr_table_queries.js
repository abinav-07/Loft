      ...
      if (result && result.length < 2) {
        ...
        pool.query(setStatusNull, (resetErr, resetResult) => {
          ...
          pool.query(insert_into_users_logs_sql, (err1, result1) => {
            if (err1) {
              ...
              res
                .status(400)
                .send("error in get /insert-users_audio_logs-data-for-retry.");
            }
        ...
        pool.query(setStatusFail, (err, result3) => {
          if (err) {
            ...
            res
              .status(400)
              .send("error in get /update-users_audio-data-for-retry.");
          }
  ...
  if (req.body.user_id) {
    ...
    pool.query(check_in_users_audio_logs, (err1, checkResult) => {
      if (err1) {
        ...
        res
          .status(400)
          .send("error in get /select-users_audio_logs-data-for-retry.");
      }
            ...
            pool.query(insert_into_users_logs_sql, (err1, result1) => {
              if (err1) {
                ...
                res
                  .status(400)
                  .send(
                    "error in get /insert-users_audio_logs-data-for-retry-transcription."
                  );
              }
            });
            //console.log(resetResult)
  ...
  if (req.body.userId) {
    ...
    pool.query(sql, (err, result) => {
      if (err) {
        ...
        res
          .status(400)
          .send("error in get /confirm-pass-fail-hr-review query.");
      }
      ...
      pool.query(check_in_users_audio_logs, (err1, result1) => {
        ...
        if (result1 && result1.length > 0) {
          var setStatusLog = `UPDATE users_audio_logs SET status="${
            req.body.changedPassFailValue
          }" WHERE users_audio_id=${req.body.userId} AND id=${
            result1[result1.length - 1].id
          }`;
...
const saveHRTestLogs = (req, res) => {
  ...
  let sql = `INSERT INTO reviewer_logs(reviewer_id,users_name,user_id,users_status,status_changed_time,type, feedback)
    VALUES ("${reviewerId}","${user_name}","${user_id}","${user_status}",Now(),"${type}", "${reviewerFeedback}")
`;
  pool.query(sql, (err, result) => {
    if (err) {
      console.error(err);
