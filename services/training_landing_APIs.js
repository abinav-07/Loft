    ...
    pool.query(get_audio_url, (err, result) => {
      ...
      if (result && result.length > 0) {
        ...
        pool.query(get_user_name, async (err, userName_result) => {
          ...
          //Check passed,failed or retry status
          let hasPassedPreviously = false;
          const checkPassFailResult = `SELECT * FROM user_language_project_type WHERE User_id=${userName_result[0]['web_app_id']} AND Project_type_id="4" AND Status_id="5"`;
          await audio_bee_pool.query(checkPassFailResult, (err, data) => {
            if (err) {
              console.error(err);
            }

            if (data && data.length > 0) {
              hasPassedPreviously = true;
            }
          });
          ...
          pool.query(check_sql, (err, result) => {
            ...
            if (result && result.length == 0) {
              ...
              if (hasPassedPreviously) {
                sql = `INSERT INTO users_audio(user_id, audio_id, start_time, end_time, status, is_submitted) VALUES('${req.query.user_id}', 
                '${audioId}', Now(), Now(), "PASS", "TRUE")`;
              } else {
                sql = `INSERT INTO users_audio(user_id, audio_id, start_time) VALUES('${req.query.user_id}', 
                '${audioId}', Now())`;
              }
              ...
              pool.query(check_in_users_audio_logs, (err, result1) => {
                if (err) {
                  ...
                  res
                    .status(400)
                    .send(
                      'error in get /training check_users_audio_logs query.'
                    );
                }
                if (result1 && result1.length > 0) {
                  ...
                  pool.query(
                    update_start_time_in_users_logs,
                    (err2, result2) => {
                      if (err2) {
                        console.error(err2);
                        res
                          .status(400)
                          .send(
                            'error in get /training update_users_audio_logs query.'
                          );
                      }
                    }
                  );
          ...
          if (result && result.length == 0) {
            ...
            pool.query(check_in_users_audio_logs, (err, result1) => {
              if (err) {
                ...
                res
                  .status(400)
                  .send('error in get /training check_users_audio_logs query.');
              }
              if (result1 && result1.length > 0) {
                var update_start_time_in_users_logs = `UPDATE users_audio_logs SET start_time=NOW()
                                 WHERE 
